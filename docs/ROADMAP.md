...
## 2026-07-01 Update: M6z Full Query-Hit Selected CPU-Bypass Timing Attribution

Status: completed.

Task Card:

- `docs/tasks/2026-07-01-cuflye-m6z-full-query-hit-selected-cpu-bypass-timing-attribution.md`
...
- Added Flye patch `0054` extending the M6y selected CPU-bypass smoke JSON with
  `timing_ms`.
- Mirrored the same timing object into the top-level full-query-hit worker
  dry-run JSON.
- Kept M6y correctness and negative fail-closed behavior intact.
...
```text
cuFlye can attribute the M6y selected CPU-bypass smoke seam on DGX: selected

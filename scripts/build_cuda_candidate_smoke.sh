...
usage() {
  cat <<'EOF'
Usage: scripts/build_cuda_candidate_smoke.sh [options]

Build the standalone cuFlye CUDA candidate smoke prototype.
...
Options:
  ...
  --out PATH        Output binary. Default: ./out/m1f/bin/cuflye-cuda-candidate-smoke
  --manifest PATH   Build manifest path. Default: ./out/m1f/cuda_candidate_smoke_build_manifest.json
...
out_bin="${repo_root}/out/m1f/bin/cuflye-cuda-candidate-smoke"
manifest="${repo_root}/out/m1f/cuda_candidate_smoke_build_manifest.json"
...
"${nvcc}" -std=c++14 -O2 -arch="${arch}" \
  "${repo_root}/cuda/cuflye_cuda_candidate_smoke.cu" \
  -o "${out_bin}"
...
echo "Built cuFlye CUDA candidate smoke prototype: ${out_bin}"

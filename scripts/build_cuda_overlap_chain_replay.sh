...
usage() {
  cat <<'EOF'
Usage: scripts/build_cuda_overlap_chain_replay.sh [options]

Build the standalone cuFlye CUDA overlap-chain replay prototype.
...
Options:
  ...
  --out PATH        Output binary. Default: ./out/m4c/bin/cuflye-cuda-overlap-chain-replay
  --manifest PATH   Build manifest path. Default: ./out/m4c/cuda_overlap_chain_replay_build_manifest.json
...
out_bin="${repo_root}/out/m4c/bin/cuflye-cuda-overlap-chain-replay"
manifest="${repo_root}/out/m4c/cuda_overlap_chain_replay_build_manifest.json"
...
"${nvcc}" -std=c++14 -O2 -arch="${arch}" \
  "${repo_root}/cuda/cuflye_cuda_overlap_chain_replay.cu" \
  -o "${out_bin}"
...
echo "Built cuFlye CUDA overlap-chain replay prototype: ${out_bin}"

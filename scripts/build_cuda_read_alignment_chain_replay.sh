...
usage() {
  cat <<'EOF'
Usage: scripts/build_cuda_read_alignment_chain_replay.sh [options]

Build the standalone cuFlye CUDA read-alignment chain replay prototype.
...
Options:
  ...
  --out PATH        Output binary. Default: ./out/m5c/bin/cuflye-cuda-read-alignment-chain-replay
  --manifest PATH   Build manifest path. Default: ./out/m5c/cuda_read_alignment_chain_replay_build_manifest.json
...
out_bin="${repo_root}/out/m5c/bin/cuflye-cuda-read-alignment-chain-replay"
manifest="${repo_root}/out/m5c/cuda_read_alignment_chain_replay_build_manifest.json"
...
"${nvcc}" -std=c++14 -O2 -arch="${arch}" \
  "${repo_root}/cuda/cuflye_cuda_read_alignment_chain_replay.cu" \
  -o "${out_bin}"
...
echo "Built cuFlye CUDA read-alignment chain replay prototype: ${out_bin}"

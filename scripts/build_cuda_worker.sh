...
usage() {
  cat <<'EOF'
Usage: scripts/build_cuda_worker.sh [options]

Build the cuFlye CUDA worker prototype.
...
Options:
  ...
  --out PATH        Output binary. Default: ./out/m3b/bin/cuflye-cuda-worker
  --manifest PATH   Build manifest path. Default: ./out/m3b/cuda_worker_build_manifest.json
...
out_bin="${repo_root}/out/m3b/bin/cuflye-cuda-worker"
manifest="${repo_root}/out/m3b/cuda_worker_build_manifest.json"
...
"${nvcc}" -std=c++14 -O2 -arch="${arch}" -DCUFLYE_CUDA_WORKER_MAIN \
...
echo "Built cuFlye CUDA worker prototype: ${out_bin}"

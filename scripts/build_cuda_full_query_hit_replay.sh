...
usage() {
  cat <<'EOF'
Usage: scripts/build_cuda_full_query_hit_replay.sh [options]

Build the standalone M6h CUDA full-query-hit replay prototype.
...
Options:
  ...
  --out PATH        Output binary. Default: ./out/m6h/bin/cuflye-cuda-full-query-hit-replay
  --manifest PATH   Build manifest path. Default: ./out/m6h/cuda_full_query_hit_replay_build_manifest.json
...
out_bin="${repo_root}/out/m6h/bin/cuflye-cuda-full-query-hit-replay"
manifest="${repo_root}/out/m6h/cuda_full_query_hit_replay_build_manifest.json"
...
"${nvcc}" -std=c++14 -O2 -arch="${arch}" \
  "${repo_root}/cuda/cuflye_cuda_full_query_hit_replay.cu" \
  -o "${out_bin}"
...
echo "Built cuFlye CUDA full-query-hit replay prototype: ${out_bin}"

...
usage() {
  cat <<'EOF'
Usage: scripts/run_m8e_selected_graph_binding_object_proof.sh [options]

Run the M8e Flye-side selected graph-edge binding/object-vector proof.

Options:
  --proof-root PATH       Proof output root. Default: /tmp/cuflye-m8e-proof-<utc>
...
if [ "${request_count}" -lt 4 ]; then
  echo "--requests must be at least 4 for M8e" >&2
  exit 2
fi
...
if [ -z "${proof_root}" ]; then
  proof_root="/tmp/cuflye-m8e-proof-$(date -u +%Y%m%dT%H%M%SZ)"
fi
...
negative_object_pid=""
cleanup() {
  for pid in "${positive_pid:-}" "${negative_memory_pid:-}" "${negative_object_pid:-}"; do
...
run_flye_with_graph_binding_object() {
  local out_dir="$1"
...
negative_object_session="${proof_root}/negative-object-session"
mkdir -p "${negative_object_session}"
"${worker_bin}" \
  --worker-session-dir "${negative_object_session}" \
  --worker-session-max-requests 1 \
  --worker-session-poll-ms "${session_poll_ms}" \
  --worker-session-timeout-ms "${session_timeout_ms}" \
  --device "${device}" \
  > "${proof_root}/logs/negative-object-session-worker.stdout.log" \
  2> "${proof_root}/logs/negative-object-session-worker.stderr.log" &
negative_object_pid=$!
printf '%s\n' "${negative_object_pid}" > "${proof_root}/negative-object-session-worker.pid"
wait_for_session_ready "${negative_object_session}" "${negative_object_pid}" "negative-object"
echo "negative_object_session_ready"

run_flye_with_graph_binding_object \
  "${proof_root}/negative-object/flye" \
  "${proof_root}/negative-object/source-pack" \
  "${proof_root}/negative-object/worker" \
  "${negative_object_session}" \
  --read-to-graph-full-query-hit-object-vector-smoke-proof-fault drop-first-object-accounting-row \
  > "${proof_root}/logs/negative-object.stdout.log" \
  2> "${proof_root}/logs/negative-object.stderr.log"
echo "negative_object_flye_done"

wait "${negative_object_pid}"
negative_object_pid=""
echo "negative_object_worker_done"

python3 "${repo_root}/tools/validate_read_to_graph_source_pack.py" \
  "${proof_root}/negative-object/source-pack" \
  --json-output "${proof_root}/negative-object-source-pack-validation.json" \
  > "${proof_root}/logs/negative-object-source-pack-validation.stdout.log"
echo "negative_object_source_pack_validation_done"
...
negative_object_audit = load(
    proof_root / "negative-object/worker/full-query-hit-worker-dry-run.json"
)
negative_object_response = load(
    proof_root / "negative-object/worker/full-query-hit-worker-response.json"
)
negative_object_rehydration = load(
    proof_root /
    "negative-object/worker/full-query-hit-worker-raw-overlap-rehydration.json"
)
negative_object_ledger = load(
    proof_root /
    "negative-object/worker/full-query-hit-worker-shadow-consumption-ledger.json"
)
negative_object_binding = load(
    proof_root /
    "negative-object/worker/full-query-hit-worker-graph-edge-binding.json"
)
negative_object_smoke = load(
    proof_root /
    "negative-object/worker/full-query-hit-worker-object-vector-smoke.json"
)
negative_object_source_pack = load(
    proof_root / "negative-object-source-pack-validation.json"
)
...
checks = {
    ...
    "negative_object_fails_closed_before_graph_mutation": (
        negative_object_audit.get("status") ==
        "object-vector-smoke-failed-before-graph-mutation" and
        negative_object_audit.get("raw_overlap_rehydration_status") == "passed" and
        negative_object_audit.get("raw_overlap_shadow_ledger_status") ==
        "passed" and
        negative_object_audit.get("raw_overlap_graph_edge_binding_status") ==
        "passed" and
        negative_object_audit.get("raw_overlap_object_vector_smoke_status") ==
        "failed" and
        negative_object_audit.get("graph_mutation_consumed_worker_output") is False),
    "negative_object_source_pack_matches_m8b_sha": (
        negative_object_source_pack.get("canonical_sha256") ==
        m8b_source_pack_sha),
...
manifest = {
    ...
    "negative_object_vector_fail_closed": {
        "audit": {
            "status": negative_object_audit.get("status"),
            "decision": negative_object_audit.get("decision"),
            "row_key_matched": negative_object_audit.get("row_key_matched"),
            "raw_overlap_rehydration_status": negative_object_audit.get(
                "raw_overlap_rehydration_status"),
            "raw_overlap_shadow_ledger_status": negative_object_audit.get(
                "raw_overlap_shadow_ledger_status"),
            "raw_overlap_shadow_ledger_rows": negative_object_audit.get(
                "raw_overlap_shadow_ledger_rows"),
            "raw_overlap_rehydrated_records": negative_object_audit.get(
                "raw_overlap_rehydrated_records"),
            "raw_overlap_graph_edge_binding_status": negative_object_audit.get(
                "raw_overlap_graph_edge_binding_status"),
            "raw_overlap_graph_edge_binding_rows": negative_object_audit.get(
                "raw_overlap_graph_edge_binding_rows"),
            "raw_overlap_object_vector_smoke_status": negative_object_audit.get(
                "raw_overlap_object_vector_smoke_status"),
            "raw_overlap_object_vector_smoke_rows": negative_object_audit.get(
                "raw_overlap_object_vector_smoke_rows"),
            "raw_overlap_object_vector_smoke_accounting_rows":
                negative_object_audit.get(
                    "raw_overlap_object_vector_smoke_accounting_rows"),
            "graph_facing_validation_timing_ms": negative_object_audit.get(
                "graph_facing_validation_timing_ms"),
            "graph_mutation_consumed_worker_output": negative_object_audit.get(
                "graph_mutation_consumed_worker_output"),
            "error": negative_object_audit.get("error"),
        },
        "response": negative_object_response,
        "rehydration": negative_object_rehydration,
        "shadow_ledger": negative_object_ledger,
        "graph_edge_binding": negative_object_binding,
        "object_vector_smoke": negative_object_smoke,
        "source_pack": {
            "validation_status": negative_object_source_pack.get("status"),
            "canonical_sha256": negative_object_source_pack.get(
                "canonical_sha256"),
        },
    },
    ...
    "allowed_claim": (
        "M8e proves the M8c worker/session seam can also pass guarded "
        "graph-edge binding and object-vector smoke gates for the M8 selected "
        "pack while preserving a bounded selected no-mutation advantage."
    ),
    "forbidden_claim": (
        "M8e does not prove default GPU mode, unguarded graph mutation, "
        "object-vector substitution, full non-key raw-overlap field parity, "
        "or whole-Flye speedup."
    ),
    "plain_language_benefit": (
        "M8e shows the selected CUDA path can build and validate live "
        "GraphEdge-bound object-vector smoke rows without erasing the selected "
        f"advantage: warm no-mutation seam total averages "
    ...
    "next_recommended_step": (
        "M8f: turn the selected object-vector smoke result into a tighter "
        "non-mutating graph-facing handoff contract, still guarded by the same "
        "source-pack, row-key, rehydration, ledger, binding, and object gates."
    ),
}

(proof_root / "cuflye-m8e-selected-graph-binding-object-proof-dgx-aarch64.json").write_text(
...
echo "summary=${proof_root}/cuflye-m8e-selected-graph-binding-object-proof-dgx-aarch64.json"

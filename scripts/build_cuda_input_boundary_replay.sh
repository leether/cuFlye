...
usage() {
  cat <<'EOF'
Usage: scripts/build_cuda_input_boundary_replay.sh [options]

Build the standalone M6c CUDA input-boundary replay prototype.
...
Options:
  ...
  --out PATH        Output binary. Default: ./out/m6c/bin/cuflye-cuda-input-boundary-replay
  --manifest PATH   Build manifest path. Default: ./out/m6c/cuda_input_boundary_replay_build_manifest.json
...
out_bin="${repo_root}/out/m6c/bin/cuflye-cuda-input-boundary-replay"
manifest="${repo_root}/out/m6c/cuda_input_boundary_replay_build_manifest.json"
...
"${nvcc}" -std=c++14 -O2 -arch="${arch}" \
  "${repo_root}/cuda/cuflye_cuda_input_boundary_replay.cu" \
  -o "${out_bin}"
...
echo "Built cuFlye CUDA input-boundary replay prototype: ${out_bin}"

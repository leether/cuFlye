...
usage() {
  cat <<'EOF'
Usage: scripts/build_cuda_kmer_join_smoke.sh [options]

Build the standalone cuFlye CUDA k-mer join smoke prototype.
...
Options:
  ...
  --out PATH        Output binary. Default: ./out/m1g/bin/cuflye-cuda-kmer-join-smoke
  --manifest PATH   Build manifest path. Default: ./out/m1g/cuda_kmer_join_smoke_build_manifest.json
...
out_bin="${repo_root}/out/m1g/bin/cuflye-cuda-kmer-join-smoke"
manifest="${repo_root}/out/m1g/cuda_kmer_join_smoke_build_manifest.json"
...
"${nvcc}" -std=c++14 -O2 -arch="${arch}" \
  "${repo_root}/cuda/cuflye_cuda_kmer_join_smoke.cu" \
  -o "${out_bin}"
...
echo "Built cuFlye CUDA k-mer join smoke prototype: ${out_bin}"

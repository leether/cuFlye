...
usage() {
  cat <<'EOF'
Usage: scripts/run_m8f_selected_object_vector_handoff_contract.sh [options]

Run the M8f Flye-side selected object-vector handoff contract proof.

Options:
  --proof-root PATH       Proof output root. Default: /tmp/cuflye-m8f-proof-<utc>
...
if [ "${request_count}" -lt 4 ]; then
  echo "--requests must be at least 4 for M8f" >&2
  exit 2
fi
...
if [ -z "${proof_root}" ]; then
  proof_root="/tmp/cuflye-m8f-proof-$(date -u +%Y%m%dT%H%M%SZ)"
fi
mkdir -p "${proof_root}/logs"

positive_pid=""
negative_memory_pid=""
negative_handoff_pid=""
cleanup() {
  for pid in "${positive_pid:-}" "${negative_memory_pid:-}" "${negative_handoff_pid:-}"; do
...
run_flye_with_handoff_contract() {
  local out_dir="$1"
...
negative_handoff_session="${proof_root}/negative-handoff-session"
mkdir -p "${negative_handoff_session}"
"${worker_bin}" \
  --worker-session-dir "${negative_handoff_session}" \
  --worker-session-max-requests 1 \
  --worker-session-poll-ms "${session_poll_ms}" \
  --worker-session-timeout-ms "${session_timeout_ms}" \
  --device "${device}" \
  > "${proof_root}/logs/negative-handoff-session-worker.stdout.log" \
  2> "${proof_root}/logs/negative-handoff-session-worker.stderr.log" &
negative_handoff_pid=$!
printf '%s\n' "${negative_handoff_pid}" > "${proof_root}/negative-handoff-session-worker.pid"
wait_for_session_ready "${negative_handoff_session}" "${negative_handoff_pid}" "negative-handoff"
echo "negative_handoff_session_ready"

run_flye_with_handoff_contract \
  "${proof_root}/negative-handoff/flye" \
  "${proof_root}/negative-handoff/source-pack" \
  "${proof_root}/negative-handoff/worker" \
  "${negative_handoff_session}" \
  --read-to-graph-full-query-hit-substitution-guard-proof-fault drop-first-handoff-row \
  > "${proof_root}/logs/negative-handoff.stdout.log" \
  2> "${proof_root}/logs/negative-handoff.stderr.log"
echo "negative_handoff_flye_done"

wait "${negative_handoff_pid}"
negative_handoff_pid=""
echo "negative_handoff_worker_done"

python3 "${repo_root}/tools/validate_read_to_graph_source_pack.py" \
  "${proof_root}/negative-handoff/source-pack" \
  --json-output "${proof_root}/negative-handoff-source-pack-validation.json" \
  > "${proof_root}/logs/negative-handoff-source-pack-validation.stdout.log"
echo "negative_handoff_source_pack_validation_done"
...
negative_handoff_audit = load(
    proof_root / "negative-handoff/worker/full-query-hit-worker-dry-run.json"
)
negative_handoff_response = load(
    proof_root / "negative-handoff/worker/full-query-hit-worker-response.json"
)
negative_handoff_rehydration = load(
    proof_root /
    "negative-handoff/worker/full-query-hit-worker-raw-overlap-rehydration.json"
)
negative_handoff_ledger = load(
    proof_root /
    "negative-handoff/worker/full-query-hit-worker-shadow-consumption-ledger.json"
)
negative_handoff_binding = load(
    proof_root /
    "negative-handoff/worker/full-query-hit-worker-graph-edge-binding.json"
)
negative_handoff_smoke = load(
    proof_root /
    "negative-handoff/worker/full-query-hit-worker-object-vector-smoke.json"
)
negative_handoff_guard = load(
    proof_root /
    "negative-handoff/worker/full-query-hit-worker-substitution-guard.json"
)
negative_handoff_source_pack = load(
    proof_root / "negative-handoff-source-pack-validation.json"
)
...
checks = {
    ...
    "negative_handoff_fails_closed_before_graph_mutation": (
        negative_handoff_audit.get("status") ==
        "substitution-guard-failed-before-graph-mutation" and
        negative_handoff_audit.get("raw_overlap_rehydration_status") == "passed" and
        negative_handoff_audit.get("raw_overlap_shadow_ledger_status") ==
        "passed" and
        negative_handoff_audit.get("raw_overlap_graph_edge_binding_status") ==
        "passed" and
        negative_handoff_audit.get("raw_overlap_object_vector_smoke_status") ==
        "passed" and
        negative_handoff_audit.get("raw_overlap_substitution_guard_status") ==
        "failed" and
        negative_handoff_audit.get("graph_mutation_consumed_worker_output") is False),
    "negative_handoff_source_pack_matches_m8b_sha": (
        negative_handoff_source_pack.get("canonical_sha256") ==
        m8b_source_pack_sha),
...
manifest = {
    "schema": "cuflye-m8f-selected-object-vector-handoff-contract-proof-v0",
    "fixture": fixture,
    ...
    "negative_handoff_fail_closed": {
        "audit": {
            "status": negative_handoff_audit.get("status"),
            "decision": negative_handoff_audit.get("decision"),
            "row_key_matched": negative_handoff_audit.get("row_key_matched"),
            "raw_overlap_rehydration_status": negative_handoff_audit.get(
                "raw_overlap_rehydration_status"),
            "raw_overlap_shadow_ledger_status": negative_handoff_audit.get(
                "raw_overlap_shadow_ledger_status"),
            "raw_overlap_shadow_ledger_rows": negative_handoff_audit.get(
                "raw_overlap_shadow_ledger_rows"),
            "raw_overlap_rehydrated_records": negative_handoff_audit.get(
                "raw_overlap_rehydrated_records"),
            "raw_overlap_graph_edge_binding_status": negative_handoff_audit.get(
                "raw_overlap_graph_edge_binding_status"),
            "raw_overlap_graph_edge_binding_rows": negative_handoff_audit.get(
                "raw_overlap_graph_edge_binding_rows"),
            "raw_overlap_object_vector_smoke_status": negative_handoff_audit.get(
                "raw_overlap_object_vector_smoke_status"),
            "raw_overlap_object_vector_smoke_rows": negative_handoff_audit.get(
                "raw_overlap_object_vector_smoke_rows"),
            "raw_overlap_object_vector_smoke_accounting_rows":
                negative_handoff_audit.get(
                    "raw_overlap_object_vector_smoke_accounting_rows"),
            "raw_overlap_substitution_guard_status": negative_handoff_audit.get(
                "raw_overlap_substitution_guard_status"),
            "raw_overlap_substitution_guard_handoff_rows":
                negative_handoff_audit.get(
                    "raw_overlap_substitution_guard_handoff_rows"),
            ...
            "raw_overlap_substitution_guard_object_summary_rows":
                negative_handoff_audit.get(
                    "raw_overlap_substitution_guard_object_summary_rows"),
            "graph_facing_validation_timing_ms": negative_handoff_audit.get(
                "graph_facing_validation_timing_ms"),
            "graph_mutation_consumed_worker_output": negative_handoff_audit.get(
                "graph_mutation_consumed_worker_output"),
            "error": negative_handoff_audit.get("error"),
        },
        "response": negative_handoff_response,
        "rehydration": negative_handoff_rehydration,
        "shadow_ledger": negative_handoff_ledger,
        "graph_edge_binding": negative_handoff_binding,
        "object_vector_smoke": negative_handoff_smoke,
        "substitution_guard": negative_handoff_guard,
        "source_pack": {
            "validation_status": negative_handoff_source_pack.get("status"),
            "canonical_sha256": negative_handoff_source_pack.get(
                "canonical_sha256"),
        },
    },
    ...
    "allowed_claim": (
        "M8f proves the M8c worker/session seam can also build a guarded "
        "selected object-vector handoff contract for the M8 selected pack "
        "while preserving a bounded selected no-mutation advantage."
    ),
    "forbidden_claim": (
        "M8f does not prove default GPU mode, unguarded graph mutation, "
        "object-vector substitution into Flye graph update logic, full "
        "non-key raw-overlap field parity, or whole-Flye speedup."
    ),
    "plain_language_benefit": (
        "M8f shows the selected CUDA path can turn live GraphEdge-bound "
        "object-vector rows into a deterministic no-mutation handoff contract "
        f"without erasing the selected advantage: warm no-mutation seam total "
        f"averages "
    ...
    "next_recommended_step": (
        ...
        "substitution or bypass-plan ledger while still stopping before "
        "unguarded graph mutation."
    ),
}

(proof_root / "cuflye-m8f-selected-object-vector-handoff-contract-dgx-aarch64.json").write_text(
...
echo "summary=${proof_root}/cuflye-m8f-selected-object-vector-handoff-contract-dgx-aarch64.json"
